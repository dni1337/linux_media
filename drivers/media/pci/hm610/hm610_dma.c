...
int sg_dma_init(struct hm610_dev *dev) 
{
	...
	gDataBuffer = dma_alloc_coherent(&dev->pci_dev->dev,
			SG_DMA_PAGE_SIZE, &gDataBufferHW, GFP_KERNEL); 
		
	
	gBDBuffer = dma_alloc_coherent(&dev->pci_dev->dev,
			2<<20, &gBDBufferHW, GFP_KERNEL); 
